...
usekey&&(a=encryptMessage(a));conn.send(JSON.stringify({a:"message",msg:a}));appendMessage(b);scrollToBottom()}}function scrollToBottom(){$("html, body").animate({scrollTop:$(document).height()-$(window).height()})}function removeErrorMessages(){var a=$("#error");0<a.size()&&a.each(function(a,c){$(c).remove()})}threshold=2E3;lastTyping=parseInt((new Date).getTime())-threshold;
function handleMessage(a){isLoggedIn&&(jsonObj=JSON.parse(a),"message"===jsonObj.a&&"typing"===jsonObj.t?1>$(".from-"+jsonObj.from).size()&&$.jGrowl(jsonObj.msg,{life:3500,group:"from-"+jsonObj.from}):"message"===jsonObj.a&&"status-message"===jsonObj.t?$.jGrowl(jsonObj.msg,{life:1500,group:"from-status-"+jsonObj.from}):"message"===jsonObj.a&&"status"===jsonObj.t?(appendMessage(jsonObj.msg),!windowFocused&&"message"===jsonObj.t&&Tinycon.setBubble(++messageCount)):"message"===jsonObj.a&&"message"===
jsonObj.t?(0<$(".from-"+jsonObj.from).size()&&(lastTyping=parseInt((new Date).getTime())-threshold,$(".from-"+jsonObj.from).remove()),(new Audio("cdn/sounds/notification.ogg")).play(),usekey&&(jsonObj.msg=decryptMessage(jsonObj.msg)),appendMessage(jsonObj.msg),!windowFocused&&"message"===jsonObj.t&&Tinycon.setBubble(++messageCount)):"login"===jsonObj.a&&(jsonObj.isLoggedIn?(removeErrorMessages(),$("#login-form").prepend('<div id="error"></div>'),$("#error").addClass("alert alert-warning fade in").append('<button id="close">&times;</button>'),
$("#close").addClass("close").attr({type:"button","data-dismiss":"alert"}).after("Username already taken")):(a=$("#message-form").html(),a=Handlebars.compile(a)({room:room,username:username}),$("#login-form").replaceWith(a),$("#message").focus(),$("#message").keypress(function(a){if(13==a.which)return a.preventDefault(),sendMessage($("#message").val()),$("#message").val(""),$("#message").focus(),!1;var a=parseInt((new Date).getTime()),c=a-threshold;a-threshold>lastTyping&&(console.log("is test > last: "+
c+" > "+lastTyping),sendTyping(),lastTyping=a)}),applyLogoutEvent(),applyWhoEvent(),applyClearMessagesEvent(),applyChangeStatusEvent(),$(".btn-tooltip").tooltip())))}function sendTyping(){conn.send(JSON.stringify({a:"typing"}))}function appendMessage(a){$(".messages").append('<div class="well well-sm message">'+a+"</div>")}usekey=!1;secret="";
...
!1}else room="public";$("#usekey").is(":checked")?(usekey=!0,secret=$("#secret").val()):(usekey=!1,secret="");window.location.hash=room+"@"+username;startConnection(room,username)}else return removeErrorMessages(),$("#login-form").prepend('<div id="error"></div>'),$("#error").addClass("alert alert-warning fade in").append('<button id="close">&times;</button>'),$("#close").addClass("close").attr({type:"button","data-dismiss":"alert"}).after("Usernames must be 1-16 of these characters: 0-9a-zA-Z_-."),
!1}function applyLogoutEvent(){$("#logout").on("click",function(){logout()})}function logout(){usekey=isLoggedIn=!1;secret="";conn.send(JSON.stringify({a:"logout"}));window.location.href=window.location.protocol+"//"+window.location.hostname+window.location.pathname}function applyChangeStatusEvent(){$(".chg-status").on("click",function(){var a=$(this).text();sendChangeStatus(a);$("#message").focus();scrollToBottom()})}
function sendChangeStatus(a){$("#current-status").text(a);conn.send(JSON.stringify({a:"statusChange",status:a}))}function getStatus(){return $("#current-status").text()}function applyWhoEvent(){$("#who").on("click",function(){who()})}function applyClearMessagesEvent(){$("#clear-messages").on("click",function(){clearMessages()})}function clearMessages(){var a=$(".message");0<a.size()&&(a.each(function(a,c){$(c).remove()}),who())}
function who(){conn.send(JSON.stringify({a:"who"}));$("#message").focus();scrollToBottom()}function strip_tags(a,b){b=(((b||"")+"").toLowerCase().match(/<[a-z][a-z0-9]*>/g)||[]).join("");return a.replace(/<\!--[\s\S]*?--\>|<\?(?:php)?[\s\S]*?\?>/gi,"").replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi,function(a,d){return-1<b.indexOf("<"+d.toLowerCase()+">")?a:""})}function loginToRoom(a,b){try{conn.send(JSON.stringify({a:"login",room:a,username:b}))}catch(c){}}connected=!1;
...
!1;$("#message").remove();if(!reConnecting){$("footer").html('<div id="login-form"><\!--add form on reconnect--\></div>');var a=$("#connection-lost-msg").html(),a=Handlebars.compile(a),b={timestamp:getTimestamp()},a=a(b);appendMessage(a);reConnect()}},conn.onmessage=function(a){isLoggedIn&&handleMessage(a.data)})}reConnecting=!1;
...
function(){var a=$("#secret").val().length;32===a?$("#key-length").css("color","green"):$("#key-length").css("color","red");$("#key-length").html("Key length is "+a+" characters")});$(window).on("unload",function(){return confirm("Are you sure you want to logout?")});windowFocused=!0;messageCount=0;$(window).focus(function(){windowFocused=!0;messageCount=0;Tinycon.setBubble(messageCount);idle&&resetIdleStatus()}).blur(function(){windowFocused=!1});$(window).on("mousemove, scroll, resize, keypress",
