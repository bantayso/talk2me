...
startConnection(room,username)}else return removeErrorMessages(),$("#login-form").prepend('<div id="error"></div>'),$("#error").addClass("alert alert-warning fade in").append('<button id="close">&times;</button>'),$("#close").addClass("close").attr({type:"button","data-dismiss":"alert"}).after("Usernames must be 1-16 of these characters: 0-9a-zA-Z_-."),!1}function applyLogoutEvent(){$("#logout").on("click",function(){logout()})}
...
function getStatus(){return $("#current-status").text()}function applyWhoEvent(){$("#who").on("click",function(){who()})}function applyClearMessagesEvent(){$("#clear-messages").on("click",function(){clearMessages()})}function clearMessages(){var a=$(".message");0<a.size()&&(a.each(function(a,c){$(c).remove()}),who())}function who(){conn.send(JSON.stringify({a:"who"}));$("#message").focus();scrollToBottom()}
