...
function sendChangeStatus(a){$("#current-status").text(a);conn.send(JSON.stringify({a:"statusChange",status:a}))}function applyWhoEvent(){$("#who").on("click",function(){who()})}function who(){conn.send(JSON.stringify({a:"who"}));$("#message").focus();scrollToBottom()}
...
function autoSetStatus(){var a=Math.round((new Date).getTime()/1E3)-lastActive;!idle&&a>idleInSeconds&&(sendChangeStatus("Idle"),idle=!0);setTimeout("autoSetStatus()",5E3)}function resetIdleStatus(){idle&&(idle=!1,lastActive=Math.round((new Date).getTime()/1E3),sendChangeStatus("Free"))}
