...
usekey&&(a=encryptMessage(a));conn.send(JSON.stringify({a:"message",msg:a}));appendMessage(b);scrollToBottom()}}function scrollToBottom(){$("html, body").animate({scrollTop:$(document).height()-$(window).height()})}function removeErrorMessages(){var a=$("#error");0<a.size()&&a.each(function(a,c){$(c).remove()})}threshold=3;lastTyping=parseInt((new Date).getTime())-threshold;
function handleMessage(a){isLoggedIn&&(jsonObj=JSON.parse(a),"message"===jsonObj.a&&"typing"===jsonObj.t?1>$(".from-"+jsonObj.from).size()&&$.jGrowl(jsonObj.msg,{life:1500,group:"from-"+jsonObj.from}):"message"===jsonObj.a?("message"===jsonObj.t&&((new Audio("cdn/sounds/notification.ogg")).play(),usekey&&(jsonObj.msg=decryptMessage(jsonObj.msg))),appendMessage(jsonObj.msg),!windowFocused&&"message"===jsonObj.t&&Tinycon.setBubble(++messageCount)):"login"===jsonObj.a&&(jsonObj.isLoggedIn?(removeErrorMessages(),
$("#login-form").prepend('<div id="error"></div>'),$("#error").addClass("alert alert-warning fade in").append('<button id="close">&times;</button>'),$("#close").addClass("close").attr({type:"button","data-dismiss":"alert"}).after("Username already taken")):(a=$("#message-form").html(),a=Handlebars.compile(a)({room:room,username:username}),$("#login-form").replaceWith(a),$("#message").focus(),$("#message").keypress(function(a){if(13==a.which)return a.preventDefault(),sendMessage($("#message").val()),
$("#message").val(""),$("#message").focus(),!1;a=parseInt((new Date).getTime());console.log("(threshold, lastTyping, curTyping, currTyping-threshold>lastTyping) = ("+threshold+", "+lastTyping+", "+a+", "+(a-threshold)+">"+lastTyping+")");a-threshold>lastTyping&&sendTyping();lastTyping=a}),applyLogoutEvent(),applyWhoEvent(),applyClearMessagesEvent(),applyChangeStatusEvent(),$(".btn-tooltip").tooltip())))}function sendTyping(){conn.send(JSON.stringify({a:"typing"}))}
function appendMessage(a){$(".messages").append('<div class="well well-sm message">'+a+"</div>")}usekey=!1;secret="";
