...
reConnect())},conn.onmessage=function(a){isLoggedIn&&handleMessage(a.data)})}reConnecting=!1;function reConnect(){reConnecting=!0;connected||(1>$("#room").size()&&$("body").append('<input id="room" type="hidden" />'),1>$("#username").size()&&$("body").append('<input id="username" type="hidden" />'),init(),setTimeout("reConnect()",2E3))}idle=!1;lastActive=Math.round((new Date).getTime()/1E3);
function autoSetStatus(){var a=Math.round((new Date).getTime()/1E3)-lastActive;!idle&&(a>idleInSeconds&&"Free"===getStatus())&&(sendChangeStatus("Idle"),idle=!0);setTimeout("autoSetStatus()",5E3)}function resetIdleStatus(){idle&&(idle=!1,lastActive=Math.round((new Date).getTime()/1E3),sendChangeStatus("Free"))}
