...
usekey&&(a=encryptMessage(a));conn.send(JSON.stringify({a:"message",msg:a}));appendMessage(b);scrollToBottom()}}function scrollToBottom(){$("html, body").animate({scrollTop:$(document).height()-$(window).height()})}function removeErrorMessages(){var a=$("#error");0<a.size()&&a.each(function(a,c){$(c).remove()})}
...
function removeRoomMember(a){a=$(".room-user[data-username='"+a+"'");0<a.size()&&a.remove()}function updateRoomMembers(a){var b="",c;for(c in a)1>$(".room-user[data-username='"+c+"'").size()&&(b+='<span class="room-user" data-username="'+c+'">'+a[c]+"</span>");$("#users-online").html(b)}threshold=2E3;lastTyping=parseInt((new Date).getTime())-threshold;
...
group:"from-status-"+jsonObj.from})):"message"===jsonObj.a&&"who"===jsonObj.t?updateRoomMembers(jsonObj.users):"message"===jsonObj.a&&"message"===jsonObj.t?(0<$(".from-"+jsonObj.from).size()&&(lastTyping=parseInt((new Date).getTime())-threshold,$(".from-"+jsonObj.from).remove()),(new Audio("cdn/sounds/notification.ogg")).play(),usekey&&(jsonObj.msg=decryptMessage(jsonObj.msg)),appendMessage(jsonObj.msg),!windowFocused&&"message"===jsonObj.t&&Tinycon.setBubble(++messageCount)):"login"===jsonObj.a&&
(jsonObj.isLoggedIn?(removeErrorMessages(),$("#login-form").prepend('<div id="error"></div>'),$("#error").addClass("alert alert-warning fade in").append('<button id="close">&times;</button>'),$("#close").addClass("close").attr({type:"button","data-dismiss":"alert"}).after("Username already taken")):(a=$("#message-form").html(),a=Handlebars.compile(a)({room:room,username:username}),$("#login-form").replaceWith(a),$("#message").focus(),$("#message").keypress(function(a){if(13==a.which)return a.preventDefault(),
