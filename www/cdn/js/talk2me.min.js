...
!1}function applyLogoutEvent(){$("#logout").on("click",function(){logout()})}function logout(){isLoggedIn=!1;conn.send(JSON.stringify({a:"logout"}));window.location.href=window.location.protocol+"//"+window.location.hostname+window.location.pathname}
...
reConnect()},conn.onmessage=function(a){isLoggedIn&&handleMessage(a.data)})}
function reConnect(){conn=new WebSocket(webSocketUrl);connected||(conn.onopen=function(){console.log("reConnect: opened");connected=!0});connected?(console.log("reConnect: connected - run init()"),1>$("#room").size()&&$("body").append('<input id="room" type="hidden" />'),1>$("#username").size()&&$("body").append('<input id="username" type="hidden" />'),init()):(console.log("reConnect: not connected, run reConnect() again"),setTimeout("reConnect()",2E3))}
