...
$("#message").focus(),$("#message").keypress(function(a){if(13==a.which)return a.preventDefault(),sendMessage($("#message").val()),$("#message").val(""),$("#message").focus(),!1}),applyLogoutEvent(),applyWhoEvent(),$(".btn-tooltip").tooltip())))}function appendMessage(a){$(".messages").append('<div class="well well-sm">'+a+"</div>")}
...
function strip_tags(a,b){b=(((b||"")+"").toLowerCase().match(/<[a-z][a-z0-9]*>/g)||[]).join("");return a.replace(/<\!--[\s\S]*?--\>|<\?(?:php)?[\s\S]*?\?>/gi,"").replace(/<\/?([a-z][a-z0-9]*)\b[^>]*>/gi,function(a,d){return-1<b.indexOf("<"+d.toLowerCase()+">")?a:""})}function loginToRoom(a,b){conn.send(JSON.stringify({a:"login",room:a,username:b}))}connected=!1;
